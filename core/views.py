...
def app_icon(request):
    """The school's logo rendered as a SQUARE PNG app icon (?size=192 or 512).
    ANY uploaded image — any shape or size — is fitted, centered and padded onto
    a white square, so it always works as a phone launcher icon (the school
    doesn't have to prepare a perfect square). Falls back to the generic Roshni
    icon if there's no logo or Pillow isn't installed. Public (no login) so the
    browser / PWABuilder can fetch it while building the install icon."""
    ...
    try:
        ...
        size = 512
    size = 512 if size >= 384 else 192
    school = School.objects.first()
    if school and getattr(school, 'logo', None):
        try:
            from PIL import Image
            with school.logo.open('rb') as fh:     # read + close (Windows locks)
                data = fh.read()
            src = Image.open(io.BytesIO(data)).convert('RGBA')
            pad = int(size * 0.08)                 # small breathing room
            src.thumbnail((size - 2 * pad, size - 2 * pad), Image.LANCZOS)
...
def web_manifest(request):
    ...
    if explicit and school and getattr(school, 'logo', None):
        try:
            import PIL  # noqa: F401  — square icons are generated by app_icon
            base = request.build_absolute_uri(reverse('app_icon'))
            icons = [
                {'src': base + '?size=192', 'sizes': '192x192',
                 'type': 'image/png', 'purpose': 'any'},
                {'src': base + '?size=512', 'sizes': '512x512',
                 'type': 'image/png', 'purpose': 'any'},
                {'src': base + '?size=512', 'sizes': '512x512',
            ...
            icons = []
    if not icons:
        # No logo (or Pillow unavailable): the generic Roshni icon set.
        icons = [
